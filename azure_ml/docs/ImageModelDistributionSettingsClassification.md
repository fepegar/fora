...
------------ | ------------- | ------------- | -------------
**training_crop_size** | Option<**String**> | Image crop size that is input to the neural network for the training dataset. Must be a positive integer. | [optional]
**validation_crop_size** | Option<**String**> | Image crop size that is input to the neural network for the validation dataset. Must be a positive integer. | [optional]
**validation_resize_size** | Option<**String**> | Image size to which to resize before cropping for validation dataset. Must be a positive integer. | [optional]
**weighted_loss** | Option<**String**> | Weighted loss. The accepted values are 0 for no weighted loss.  1 for weighted loss with sqrt.(class_weights). 2 for weighted loss with class_weights. Must be 0 or 1 or 2. | [optional]
